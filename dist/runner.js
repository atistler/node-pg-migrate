    ...
    .then(files => {
      ...
      return files
        ...
        .map(file => {
          ...
          const actions =
            _path2.default.extname(filePath) === ".sql" // eslint-disable-next-line security/detect-non-literal-fs-filename
              ? {
                  up: pgm =>
                    pgm.sql(_fs2.default.readFileSync(filePath, "utf8"))
  ...
  return db
    ...
    .then(
      migrationTables =>
        ...
        db.query(
          _utils.template`CREATE TABLE "${fullTableName}" ( id SERIAL, ${nameColumn} varchar(255) NOT NULL, ${runOnColumn} timestamp NOT NULL)`
        )
    )
    .then(() => (!options.noLock ? lock(db, options) : null))
    .then(() =>
      db.column(
        _utils.template`SELECT ${nameColumn} FROM "${fullTableName}" ORDER BY ${runOnColumn}`,
        nameColumn
      )
    )
    .catch(err => {
